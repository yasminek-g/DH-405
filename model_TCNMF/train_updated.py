...
train_images = load_and_preprocess_images(train_image_paths[:50])
...
n_components = 50  # Adjust based on desired detail
...
test_images = load_and_preprocess_images(test_image_paths[:5])
...
def display_images(original, incomplete, reconstructed, num_images=5):
    for i in range(min(num_images, original.shape[0])):
        ...
        axes[0].axis('off')
        # Incomplete image
        axes[1].imshow(incomplete[i], cmap='gray')
